#!/usr/bin/env python3
"""Flue OS — server. Stdlib only, 127.0.0.1:8876."""
import sys
...
ROUTES = {
    ...
    ("GET", "/api/households"): lambda q, b: core.due_board(),
    ("GET", "/api/households/<id>/verdict"): lambda q, b: core.burn_verdict(
        store.by_id("households", q["id"])),
    ("POST", "/api/households/<id>/report"): lambda q, b: agents.draft_report(q["id"]),
    ("GET", "/api/hazards"): lambda q, b: {"rows": core.hazard_households()},
    ("GET", "/api/season"): lambda q, b: core.season_board(),
...
if __name__ == "__main__":
    serve.run(ROOT / "app", ROUTES, PORT, "Flue OS")
