#!/usr/bin/env python3
"""Ratio OS — server. Stdlib only, 127.0.0.1:8844."""
import sys
...
def board(q, b):
    ...
    return {"config": store.load("config"),
            "board": {"ratio_over": rb["over"], "ratio_unmeasured": rb["unmeasured"],
                      "ratio_rows": rb["rows"][:12], "rules_source": rb["rules_source"],
                      "funnel": fn, "messages_open": len(msgs),
...
ROUTES = {
    ...
    ("GET", "/api/ratios"): lambda q, b: core.ratio_board(),
    ("POST", "/api/children/<id>/release"): lambda q, b: agents.check_release(q["id"], b.get("person", "")),
    ("GET", "/api/waitlist"): lambda q, b: {"rows": store.load("waitlist")[-40:],
                                            "funnel": core.funnel()},
...
if __name__ == "__main__":
    serve.run(ROOT / "app", ROUTES, PORT, "Ratio OS")
