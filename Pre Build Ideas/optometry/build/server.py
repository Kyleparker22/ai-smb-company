#!/usr/bin/env python3
"""Exam OS — server. Stdlib only, 127.0.0.1:8847."""
import sys
...
def board(q, b):
    ...
    return {"config": store.load("config"),
            "board": {"lapsed": len(lp), "lapsed_top": lp[:10], "capture": cr,
...
ROUTES = {
    ...
    ("POST", "/api/patients/<id>/reorder"): lambda q, b: core.reorder_check(
        store.by_id("patients", q["id"]) or {}),
    ("GET", "/api/capture"): lambda q, b: core.capture_rate(),
...
if __name__ == "__main__":
    serve.run(ROOT / "app", ROUTES, PORT, "Exam OS")
